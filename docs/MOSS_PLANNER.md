...
Host long-term memory follows the same rule: `memory.search`, `memory.list`, `memory.get`, and `memory.status` are read-only and may be used in safe plans, while `memory.remember` and `memory.forget` are sensitive and cannot be scheduled automatically by the planner.
