...
def test_planner_can_read_memory_but_cannot_write_it_automatically(tmp_path) -> None:
    app = planner_app(tmp_path, lambda _: httpx.Response(200, json={}))
    with TestClient(app) as client:
        read_candidate = {
            "title": "Read remembered preferences",
            "steps": [{"tool": "memory.search", "arguments": {"query": "drink"}}],
        }
        readable = client.post("/api/v1/planner/validate", json=read_candidate)
        assert readable.status_code == 200
        assert readable.json()["eligible_for_auto_mission"] is True
        ...
        writable = client.post("/api/v1/planner/validate", json=write_candidate)
        assert writable.status_code == 200
        assert writable.json()["eligible_for_auto_mission"] is False
        assert writable.json()["blocked_steps"][0]["risk"] == "sensitive"
